...
def unify_coin_jump_actions(coin_jump_actions: List[CoinJumpActions]):
...
def coin_jump_actions_from_unified(unified_coin_jump_action):
    if unified_coin_jump_action <= 4:
        return [CoinJumpActions(unified_coin_jump_action)]
    if unified_coin_jump_action < 7:
        return [CoinJumpActions(unified_coin_jump_action-4), CoinJumpActions.MOVE_UP]
    else:
        return [CoinJumpActions(unified_coin_jump_action-6), CoinJumpActions.MOVE_DOWN]
