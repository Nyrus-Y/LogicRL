...
from .key import Key
from .key2 import Key2
from .groundEnemy import GroundEnemy


class ParameterizedLevelGenerator_Keys:
    ...
    def generate(self, coinjump: CoinJump, seed=None, dynamic_reward=False, spawn_all_entities=False):
        ...
        po = np.arange(3, 23, 2, dtype=int)
        positions = [(n, 2) for n in po]
        ...
        level.entities.append(Key2(level, positions[4][0] - 0.5, positions[4][1], resource_loader=resource_loader))
