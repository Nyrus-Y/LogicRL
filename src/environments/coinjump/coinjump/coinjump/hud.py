class HUD:
    ...
    def __init__(self, coinjump, resource_loader):
        self.coinjump = coinjump
        self.resource_loader = resource_loader
        self.level = coinjump.level
    ...
    def render(self, camera, step=None):
        ...
        x += sz2 + 5
        x += self.paint_int(self.coinjump.level.reward_values['coin'], camera, x, 10, sz2)
        x += 2 * sz

        camera.paint_sprite_absolute(x, 10, self.sprites['powerup'])
        x += sz2 + 5
        x += self.paint_int(self.coinjump.level.reward_values['powerup'], camera, x, 10, sz2)
        x += 2 * sz

        camera.paint_sprite_absolute(x, 10, self.sprites['enemy'])
        x += sz2 + 5
        x += self.paint_int(self.coinjump.level.reward_values['enemy'], camera, x, 10, sz2)
        x += 2 * sz

        camera.paint_sprite_absolute(x, 10, self.sprites['key'])
        x += sz2 + 5
        x += self.paint_int(self.coinjump.level.reward_values['key'], camera, x, 10, sz2)
        x += 2 * sz

        self.paint_int(self.coinjump.score, camera, camera.width - 4 * sz2, 10, sz2)
