...
class ActorCritic(nn.Module):
    def __init__(self, predictions=None, rng=None, ):
        ...
        self.actor = NSFRController(predictions=predictions, has_softmax=True)
        ...
        #                nn.Softmax(dim=-1)
        #            )
        self.critic = NSFRController(predictions=predictions, has_softmax=False, out_size=1, special=True,
                                     critic=True)
    ...
    def act(self, state, predictions, epsilon=0.0):
...
class PPO:
    ...
    def select_action(self, state, predictions, epsilon=0.0):
        # select random action with epsilon probability and policy probiability with 1-epsilon
        with torch.no_grad():
            state = torch.FloatTensor(state).to(device)
            action, action_logprob = self.policy_old.act(state, predictions, epsilon=epsilon)

        self.buffer.states.append(state)
        self.buffer.actions.append(action)
        self.buffer.logprobs.append(action_logprob)
        self.buffer.predictions.append(predictions)
    ...
    def update(self):
        ...
        old_predictions = torch.squeeze(torch.stack(self.buffer.predictions, dim=0)).detach().to(device)

        # Optimize policy for K epochs
        for _ in range(self.K_epochs):
            # Evaluating old actions and values
            logprobs, state_values, dist_entropy = self.policy.evaluate(old_states, old_actions, old_predictions)
