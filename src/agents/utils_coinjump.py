...
def extract_logic_state_coinjump(coin_jump, args, noise=False):
    ...
    states = torch.tensor(np.array(extracted_states), dtype=torch.float32, device="cuda:0", requires_grad=True).unsqueeze(0)
    return states


def extract_neural_state_coinjump(coinjump, args):
...
def preds_to_action_coinjump(action, prednames):
    """
...
def action_map_coinjump(prediction, args, prednames=None):
    ...
    if args.alg == 'ppo':
        ...
        action = preds_to_action_coinjump(prediction, prednames)
    return action
